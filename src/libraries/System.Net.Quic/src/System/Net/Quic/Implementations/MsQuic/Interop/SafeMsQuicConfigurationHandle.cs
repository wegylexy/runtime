        ...
        private static unsafe SafeMsQuicConfigurationHandle Create(QuicOptions options, QUIC_CREDENTIAL_FLAGS flags, X509Certificate? certificate, List<SslApplicationProtocol>? alpnProtocols)
        {
            ...
            var settings = new Settings
            {
                IsSetFlags =
                    ...
                    | (ulong)SettingsFlags.DatagramReceiveEnabled,
                PeerBidiStreamCount = (ushort)options.MaxBidirectionalStreams,
                PeerUnidiStreamCount = (ushort)options.MaxUnidirectionalStreams,
                BitField = options.DatagramReceiveEnabled ? (byte)SettingsBitField.DatagramReceiveEnabled : (byte)0
            };
            ...
            CredentialConfigCertificateFile file = default;
            try
            {
                ...
                {
                    config.Type = (uint)QUIC_CREDENTIAL_TYPE.FILE;
                    file.PrivateKeyFile = Marshal.StringToCoTaskMemUTF8(privateKeyFilePath);
                    file.CertificateFile = Marshal.StringToCoTaskMemUTF8(certFilePath);
                    config.Certificate = new IntPtr(&file);
                }
            ...
            {
                if (file.PrivateKeyFile != default)
                    Marshal.ZeroFreeCoTaskMemUTF8(file.PrivateKeyFile);
                if (file.CertificateFile != default)
                    Marshal.ZeroFreeCoTaskMemUTF8(file.CertificateFile);
            }
